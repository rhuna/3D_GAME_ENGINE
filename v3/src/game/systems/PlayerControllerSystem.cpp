...
void PlayerControllerSystem::Update(Application& app, World& world, float deltaTime) {
    ...
    if (input.IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && playerComponent->fireCooldownRemaining <= 0.0f) {
        ...
        if (projectile != 0) {
            const int damage = playerComponent->baseProjectileDamage + playerComponent->bonusProjectileDamage;
            world.AddComponent<ProjectileComponent>(projectile, ProjectileComponent{damage, kPlayerTeam});
