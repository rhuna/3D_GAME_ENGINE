...
void RegionEcsLoader::Rebuild(Application& app,
    ...
    if (!layout) {
        Logger::Warn("REGION", "RegionEcsLoader could not find layout for region: " + regionId + ". Falling back to scene-only spawn path.");
    } else {
        Logger::Info(
            "REGION",
    ...
    if (layout) for (std::size_t i = 0; i < layout->gatherPoints.size(); ++i) {
    ...
    if (layout) for (std::size_t i = 0; i < layout->npcSpawns.size(); ++i) {
        const Vector3 markerPos{layout->npcSpawns[i].position.x, layout->npcSpawns[i].position.y + 1.0f, layout->npcSpawns[i].position.z};
        const Entity e = spawn::SpawnStaticBox(world,
                    "npc_spawn_marker",
                    markerPos,
        ...
        if (e != 0) {
            NpcComponent npc{};
            npc.npcId = layout->npcSpawns[i].npcId.empty() ? ("runtime_npc_" + std::to_string(i)) : layout->npcSpawns[i].npcId;
    ...
    if (layout) for (std::size_t i = 0; i < layout->travelPoints.size(); ++i) {
        const Vector3 markerPos{layout->travelPoints[i].position.x, layout->travelPoints[i].position.y + 1.0f, layout->travelPoints[i].position.z};
        ...
        if (travelEntity != 0) {
            ...
            trigger.destinationRegion = layout->travelPoints[i].targetRegion.empty() ? regionId : layout->travelPoints[i].targetRegion;
            trigger.destinationSpawn = layout->travelPoints[i].destinationSpawn;
