...
constexpr float kTabGridTop = 58.0f;
constexpr float kTabGridRows = 8.0f;
constexpr float kTabColumns = 2.0f;
constexpr float kTabRowGap = 12.0f;
constexpr float kTabColumnGap = 12.0f;
constexpr float kSidebarLeftPadding = 18.0f;
// The sidebar is intentionally a little narrower now so the editable form area
// on the right has more usable room for fields, labels, and action buttons.
constexpr float kSidebarWidth = 344.0f;
constexpr float kTabWidth = 158.0f;
constexpr float kTabHeight = 42.0f;
// Input fields are wide enough for longer ids and descriptive values.
constexpr float kFieldWidth = 430.0f;
// Input boxes are slightly shorter so labels can breathe above them without the rows feeling cramped.
constexpr float kFieldHeight = 38.0f;
// Shared vertical distance between stacked field rows. We intentionally make this much larger than
// the field height so a label can sit above its box without colliding with the previous row.
constexpr float kRowStride = 76.0f;
constexpr float kColumnGap = 86.0f;
// Buttons are intentionally large so they are easy to see and click.
constexpr float kButtonWidth = 220.0f;
constexpr float kSecondColumnOffset = kFieldWidth + kColumnGap;
// Action buttons sit in their own visible column to the right of the input fields.
constexpr float kActionColumnOffset = 520.0f;
constexpr float kWideActionColumnOffset = 560.0f;
// Labels sit farther above the box so they do not bleed into the input region.
// Labels now sit closer to their own field instead of drifting upward into the previous field.
constexpr float kFieldLabelOffset = 28.0f;
...
// This helper used to draw a repeated debug overlay every time tabs, fields, and
// status text rendered. That made the builder noisy and also obscured form labels.
// We keep the helper as a harmless no-op so older call sites remain safe.
...
void DrawButtonVisual(const Rectangle& rect, const char* label, bool hovered, Color fill) {
    ...
    DrawText(label, static_cast<int>(rect.x + 12.0f), static_cast<int>(rect.y + 10.0f), 22, RAYWHITE);
}
...
bool GameBuilderPanel::Button(const Rectangle& rect, const char* label) const {
    // The builder previously used Button() only as an invisible click detector.
    // That meant the write/save/create actions technically existed in code but
    // users could not see where to click. We now draw the button here every time
    // the helper is called, then return whether the user clicked it this frame.
    const bool hovered = PointInRect(rect, GetMousePosition());
    DrawButtonVisual(rect, label, hovered, Color{34, 60, 92, 255});
...
Rectangle GameBuilderPanel::ContentRect() const {
    // The content panel begins to the right of the sidebar and leaves a little
    // extra room for the per-tab action buttons. This function must compute the
    // rectangle directly rather than calling ContentTop(), because ContentTop()
    // itself depends on ContentRect().
    const float left = m_bounds.x + kSidebarWidth + 28.0f;
    const float top = m_bounds.y + 56.0f;
    const float rightPadding = 24.0f;
    const float bottomPadding = 56.0f;
    return Rectangle{
        left,
        top,
        std::max(600.0f, m_bounds.width - (left - m_bounds.x) - rightPadding),
        std::max(120.0f, m_bounds.height - (top - m_bounds.y) - bottomPadding)
    };
}

float GameBuilderPanel::ContentTop() const {
    // This is the y coordinate where the first editable row should start. It is
    // intentionally lower than the helper text so labels and boxes do not crowd
    // the top edge of the content area.
    return ContentRect().y + 92.0f;
}
...
void GameBuilderPanel::DrawHeaderTab(const Rectangle& rect, const char* label, Tab tab) const {
    ...
    DrawText(label, static_cast<int>(rect.x + 10.0f), static_cast<int>(rect.y + 10.0f), 22, active ? RAYWHITE : LIGHTGRAY);
}
...
void GameBuilderPanel::DrawTextField(const Rectangle& rect, const char* label, const std::string& value, int fieldId) const {
    ...
    DrawBuilderVisibilityText(m_visible);
    DrawText(label, static_cast<int>(rect.x), static_cast<int>(rect.y - kFieldLabelOffset), 20, LIGHTGRAY);
    DrawRectangleRec(rect, active ? Color{32, 40, 52, 255} : Color{22, 28, 38, 255});
    DrawRectangleLinesEx(rect, 1.5f, active ? SKYBLUE : Fade(LIGHTGRAY, 0.6f));
    DrawBuilderVisibilityText(m_visible);
    DrawText(value.c_str(), static_cast<int>(rect.x + 12.0f), static_cast<int>(rect.y + 12.0f), 22, RAYWHITE);
}
...
void GameBuilderPanel::DrawStatusBar() const {
    const Rectangle bar {m_bounds.x + 10.0f, m_bounds.y + m_bounds.height - 42.0f, m_bounds.width - 20.0f, 32.0f};
    DrawRectangleRec(bar, Color{20, 24, 30, 230});
    DrawRectangleLinesEx(bar, 1.0f, Fade(SKYBLUE, 0.75f));
    DrawBuilderVisibilityText(m_visible);
    DrawText(m_status.c_str(), static_cast<int>(bar.x + 10.0f), static_cast<int>(bar.y + 6.0f), 18, RAYWHITE);
}
...
void GameBuilderPanel::UpdateTabClicks() {
    ...
    for (std::size_t i = 0; i < tabs.size(); ++i) {
        const int row = static_cast<int>(i / static_cast<int>(kTabColumns));
        const int col = static_cast<int>(i % static_cast<int>(kTabColumns));
        const Rectangle rect {
            m_bounds.x + kSidebarLeftPadding + col * (kTabWidth + kTabColumnGap),
...
void GameBuilderPanel::Update(Application& app, ContentRegistry& registry) {
    ...
    const float width = std::min(1880.0f, static_cast<float>(GetScreenWidth()) - 20.0f);
    const float height = std::min(1040.0f, static_cast<float>(GetScreenHeight()) - 20.0f);
    m_bounds = Rectangle{10.0f, 10.0f, std::max(1500.0f, width), std::max(920.0f, height)};
...
void GameBuilderPanel::HandleCreateTab(ContentRegistry& registry) {
    const float left = ContentRect().x + 20.0f;
    float y = ContentTop();
    const Rectangle sceneRect = MakeRow(left, y);
    const Rectangle prefabRect = MakeRow(left, y + kRowStride);
    const Rectangle npcRect = MakeRow(left, y + kRowStride * 2.0f);
    ...
    if (Button(MakeRow(left + kActionColumnOffset, y, kButtonWidth, 34.0f), "Write Scene")) {
        std::ostringstream out;
    ...
    if (Button(MakeRow(left + kActionColumnOffset, y + 44.0f, kButtonWidth, 34.0f), "Write Prefab")) {
        std::ostringstream out;
    ...
    if (Button(MakeRow(left + kActionColumnOffset, y + 88.0f, kButtonWidth, 34.0f), "Write NPC")) {
        std::ostringstream out;
...
void GameBuilderPanel::HandleStoryTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 5> fields {{
        ...
        {MakeRow(left, y + kRowStride), {12, &m_story.npcId}},
        {MakeRow(left, y + kRowStride * 2.0f), {13, &m_story.openingText}},
        {MakeRow(left, y + kRowStride * 3.0f), {14, &m_story.choiceOne}},
        {MakeRow(left, y + kRowStride * 4.0f), {15, &m_story.choiceTwo}}
    }};
    ...
    if (Button(MakeRow(left + kWideActionColumnOffset, y, 180.0f, 34.0f), "Write Dialogue")) {
...
void GameBuilderPanel::HandleQuestTab(ContentRegistry& registry) {
    ...
    const Rectangle titleRect = MakeRow(left, y + kRowStride);
    const Rectangle typeRect = MakeRow(left, y + kRowStride * 2.0f);
    const Rectangle targetRect = MakeRow(left, y + kRowStride * 3.0f);
    const Rectangle rewardRect = MakeRow(right, y);
    const Rectangle nextRect = MakeRow(right, y + kRowStride);
    ...
    if (Button(MakeRow(right, y + kRowStride * 2.0f, 72.0f, 30.0f), "Gold-")) m_quest.rewardGold = std::max(0, m_quest.rewardGold - 5);
    if (Button(MakeRow(right + 76.0f, y + kRowStride * 2.0f, 72.0f, 30.0f), "Gold+")) m_quest.rewardGold += 5;

    if (Button(MakeRow(right, y + kRowStride * 3.0f, 180.0f, 34.0f), "Write Quest Bundle")) {
...
void GameBuilderPanel::HandleFightTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 6> fields {{
        ...
        {MakeRow(left, y + kRowStride), {32, &m_fight.displayName}},
        {MakeRow(left, y + kRowStride * 2.0f), {33, &m_fight.prefabId}},
        {MakeRow(right, y), {34, &m_fight.behavior}},
        {MakeRow(right, y + kRowStride), {35, &m_fight.encounterId}},
        {MakeRow(right, y + kRowStride * 2.0f), {36, &m_fight.lootTable}}
    }};
    for (const auto& [rect, data] : fields) {
        if (PointInRect(rect, GetMousePosition()) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) m_activeTextField = data.first;
    ...
    if (Button(MakeRow(left, y + kRowStride * 3.0f, 72.0f, 30.0f), "Count-")) m_fight.count = std::max(1, m_fight.count - 1);
    if (Button(MakeRow(left + 76.0f, y + kRowStride * 3.0f, 72.0f, 30.0f), "Count+")) ++m_fight.count;
    if (Button(MakeRow(right, y + kRowStride * 3.0f, 72.0f, 30.0f), "Rad-")) m_fight.radius = std::max(1.0f, m_fight.radius - 1.0f);
    if (Button(MakeRow(right + 76.0f, y + kRowStride * 3.0f, 72.0f, 30.0f), "Rad+")) m_fight.radius += 1.0f;
...
void GameBuilderPanel::HandleTradeTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 5> fields {{
        ...
        {MakeRow(left, y + kRowStride), {42, &m_trade.itemName}},
        {MakeRow(left, y + kRowStride * 2.0f), {43, &m_trade.itemType}},
        {MakeRow(right, y), {44, &m_trade.merchantId}},
        {MakeRow(right, y + kRowStride), {45, &m_trade.lootId}}
    }};
    for (const auto& [rect, data] : fields) {
        if (PointInRect(rect, GetMousePosition()) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) m_activeTextField = data.first;
        if (m_activeTextField == data.first) HandleTextInput(*data.second);
    ...
    if (Button(MakeRow(left, y + kRowStride * 3.0f, 72.0f, 30.0f), "Val-")) m_trade.value = std::max(0, m_trade.value - 5);
    if (Button(MakeRow(left + 76.0f, y + kRowStride * 3.0f, 72.0f, 30.0f), "Val+")) m_trade.value += 5;
    if (Button(MakeRow(left + 152.0f, y + kRowStride * 3.0f, 72.0f, 30.0f), "Heal-")) m_trade.healAmount = std::max(0, m_trade.healAmount - 5);
    if (Button(MakeRow(left + 228.0f, y + kRowStride * 3.0f, 72.0f, 30.0f), "Heal+")) m_trade.healAmount += 5;
    if (Button(MakeRow(right, y + kRowStride * 2.0f, 72.0f, 30.0f), "Stock-")) m_trade.stockCount = std::max(1, m_trade.stockCount - 1);
    if (Button(MakeRow(right + 76.0f, y + kRowStride * 2.0f, 72.0f, 30.0f), "Stock+")) ++m_trade.stockCount;

    if (Button(MakeRow(right, y + kRowStride * 3.0f, 180.0f, 34.0f), "Write Economy Bundle")) {
...
void GameBuilderPanel::HandleWorldTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 5> fields {{
        ...
        {MakeRow(left, y + kRowStride), {52, &m_world.sceneId}},
        {MakeRow(right, y), {53, &m_world.travelId}},
        {MakeRow(right, y + kRowStride), {54, &m_world.targetRegion}},
        {MakeRow(left, y + kRowStride * 2.0f), {55, &m_world.bundleId}}
    }};
    for (const auto& [rect, data] : fields) {
        if (PointInRect(rect, GetMousePosition()) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) m_activeTextField = data.first;
        if (m_activeTextField == data.first) HandleTextInput(*data.second);
    ...
    if (Button(MakeRow(right, y + kRowStride * 2.0f, 180.0f, 34.0f), "Write World Bundle")) {
...
void GameBuilderPanel::HandleInteriorTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 6> fields {{
        ...
        {MakeRow(left, y + kRowStride), {62, &m_interior.interiorId}},
        {MakeRow(left, y + kRowStride * 2.0f), {63, &m_interior.destinationScene}},
        {MakeRow(right, y), {64, &m_interior.destinationSpawn}},
        {MakeRow(right, y + kRowStride), {65, &m_interior.roomPreset}},
        {MakeRow(right, y + kRowStride * 2.0f), {66, &m_interior.containerSet}}
    }};
    ...
    if (Button(MakeRow(left, y + kRowStride * 3.0f, 180.0f, 34.0f), "Write Interior Bundle")) {
...
void GameBuilderPanel::HandlePersistTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 8> fields {{
        ...
        {MakeRow(left, y + kRowStride), {72, &m_persist.startScene}},
        {MakeRow(left, y + kRowStride * 2.0f), {73, &m_persist.startRegion}},
        {MakeRow(left, y + kRowStride * 3.0f), {74, &m_persist.startSpawn}},
        {MakeRow(right, y), {75, &m_persist.startingQuest}},
        {MakeRow(right, y + kRowStride), {76, &m_persist.startingInventory}},
        {MakeRow(right, y + kRowStride * 2.0f), {77, &m_persist.slotId}},
        {MakeRow(right, y + kRowStride * 3.0f), {78, &m_persist.worldStateId}}
    }};
    ...
    if (Button(MakeRow(left, y + kRowStride * 4.0f, 72.0f, 30.0f), "Gold-")) m_persist.startingGold = std::max(0, m_persist.startingGold - 5);
    if (Button(MakeRow(left + 76.0f, y + kRowStride * 4.0f, 72.0f, 30.0f), "Gold+")) m_persist.startingGold += 5;
    if (Button(MakeRow(right, y + kRowStride * 4.0f, 110.0f, 30.0f), m_persist.autosave ? "Autosave ON" : "Autosave OFF")) m_persist.autosave = !m_persist.autosave;
    if (Button(MakeRow(right + 116.0f, y + kRowStride * 4.0f, 120.0f, 30.0f), m_persist.checkpoint ? "Checkpoint ON" : "Checkpoint OFF")) m_persist.checkpoint = !m_persist.checkpoint;
...
void GameBuilderPanel::HandleStartTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 7> fields {{
        ...
        {MakeRow(left, y + kRowStride), {82, &m_start.gameTitle}},
        {MakeRow(left, y + kRowStride * 2.0f), {83, &m_start.hudId}},
        {MakeRow(left, y + kRowStride * 3.0f), {84, &m_start.startupId}},
        {MakeRow(right, y), {85, &m_start.profileId}},
        {MakeRow(right, y + kRowStride), {86, &m_start.newGameScene}},
        {MakeRow(right, y + kRowStride * 2.0f), {87, &m_start.continueSlot}}
    }};
    ...
    if (Button(MakeRow(right, y + kRowStride * 3.0f, 110.0f, 30.0f), m_start.showContinue ? "Continue ON" : "Continue OFF")) m_start.showContinue = !m_start.showContinue;
    if (Button(MakeRow(right + 116.0f, y + kRowStride * 3.0f, 110.0f, 30.0f), m_start.showOptions ? "Options ON" : "Options OFF")) m_start.showOptions = !m_start.showOptions;
    if (Button(MakeRow(left, y + kRowStride * 4.0f, 110.0f, 30.0f), m_start.showGold ? "Gold ON" : "Gold OFF")) m_start.showGold = !m_start.showGold;
    if (Button(MakeRow(left + 116.0f, y + kRowStride * 4.0f, 140.0f, 30.0f), m_start.showQuestTracker ? "Quest UI ON" : "Quest UI OFF")) m_start.showQuestTracker = !m_start.showQuestTracker;
...
void GameBuilderPanel::HandleAudioTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 8> fields {{
        ...
        {MakeRow(left, y + kRowStride), {92, &m_audio.regionId}},
        {MakeRow(left, y + kRowStride * 2.0f), {93, &m_audio.dayLoop}},
        {MakeRow(left, y + kRowStride * 3.0f), {94, &m_audio.nightLoop}},
        {MakeRow(right, y), {95, &m_audio.dayMusic}},
        {MakeRow(right, y + kRowStride), {96, &m_audio.nightMusic}},
        {MakeRow(right, y + kRowStride * 2.0f), {97, &m_audio.combatMusic}},
        {MakeRow(right, y + kRowStride * 3.0f), {98, &m_audio.atmosphereId}}
    }};
    ...
    if (Button(MakeRow(left, y + kRowStride * 4.0f, 72.0f, 30.0f), "Wind-")) m_audio.windLevel = std::max(0, m_audio.windLevel - 5);
    if (Button(MakeRow(left + 76.0f, y + kRowStride * 4.0f, 72.0f, 30.0f), "Wind+")) m_audio.windLevel = std::min(100, m_audio.windLevel + 5);
    if (Button(MakeRow(left + 152.0f, y + kRowStride * 4.0f, 72.0f, 30.0f), "Wild-")) m_audio.wildlifeLevel = std::max(0, m_audio.wildlifeLevel - 5);
    if (Button(MakeRow(left + 228.0f, y + kRowStride * 4.0f, 72.0f, 30.0f), "Wild+")) m_audio.wildlifeLevel = std::min(100, m_audio.wildlifeLevel + 5);
    if (Button(MakeRow(right, y + kRowStride * 4.0f, 72.0f, 30.0f), "Fire-")) m_audio.fireLevel = std::max(0, m_audio.fireLevel - 5);
    if (Button(MakeRow(right + 76.0f, y + kRowStride * 4.0f, 72.0f, 30.0f), "Fire+")) m_audio.fireLevel = std::min(100, m_audio.fireLevel + 5);
...
void GameBuilderPanel::HandleReviewTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 4> fields {{
        ...
        {MakeRow(left, y + kRowStride), {102, &m_review.targetScene}},
        {MakeRow(right, y), {103, &m_review.targetRegion}},
        {MakeRow(right, y + kRowStride), {104, &m_review.reviewId}}
    }};
    ...
    const Rectangle notesRect = MakeRow(left, y + kRowStride * 2.0f, 510.0f, 30.0f);
    ...
    if (Button(MakeRow(left, y + kRowStride * 3.0f, 200.0f, 34.0f), "Write Review Bundle")) {
...
void GameBuilderPanel::HandleReleaseTab(Application& app, ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 9> fields {{
        ...
        {MakeRow(left, y + kRowStride), {112, &m_release.gameTitle}},
        {MakeRow(left, y + kRowStride * 2.0f), {113, &m_release.version}},
        {MakeRow(left, y + kRowStride * 3.0f), {114, &m_release.startScene}},
        {MakeRow(right, y), {115, &m_release.startRegion}},
        {MakeRow(right, y + kRowStride), {116, &m_release.profileId}},
        {MakeRow(right, y + kRowStride * 2.0f), {117, &m_release.menuId}},
        {MakeRow(right, y + kRowStride * 3.0f), {118, &m_release.hudId}},
        {MakeRow(left, y + kRowStride * 4.0f, 510.0f, 30.0f), {119, &m_release.outputFolder}}
    }};
    ...
    if (Button(MakeRow(right, y + kRowStride * 4.0f, 120.0f, 30.0f), m_release.includeSaves ? "Saves ON" : "Saves OFF")) m_release.includeSaves = !m_release.includeSaves;
    if (Button(MakeRow(right + 126.0f, y + kRowStride * 4.0f, 120.0f, 30.0f), m_release.compressExport ? "Compress ON" : "Compress OFF")) m_release.compressExport = !m_release.compressExport;
...
void GameBuilderPanel::HandleTemplatesTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 12> fields {{
        ...
        {MakeRow(left, y + kRowStride), {122, &m_template.displayName}},
        {MakeRow(left, y + kRowStride * 2.0f), {123, &m_template.projectId}},
        {MakeRow(left, y + kRowStride * 3.0f), {124, &m_template.startScene}},
        {MakeRow(right, y), {125, &m_template.startRegion}},
        {MakeRow(right, y + kRowStride), {126, &m_template.profileId}},
        {MakeRow(right, y + kRowStride * 2.0f), {127, &m_template.menuId}},
        {MakeRow(right, y + kRowStride * 3.0f), {128, &m_template.hudId}},
        {MakeRow(left, y + kRowStride * 4.0f, 510.0f, 30.0f), {129, &m_template.regions}},
        {MakeRow(left, y + kRowStride * 5.0f, 510.0f, 30.0f), {130, &m_template.quests}},
        {MakeRow(left, y + kRowStride * 6.0f, 510.0f, 30.0f), {131, &m_template.dialogue}},
...
void GameBuilderPanel::HandleShipTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 4> fields {{
        ...
        {MakeRow(left, y + kRowStride), {142, &m_ship.buildId}},
        {MakeRow(right, y), {143, &m_ship.contentLockId}},
        {MakeRow(right, y + kRowStride), {144, &m_ship.releaseGateId}}
    }};
    ...
    if (Button(MakeRow(left, y + kRowStride * 2.0f, 150.0f, 30.0f), m_ship.contentLock ? "Lock ON" : "Lock OFF")) m_ship.contentLock = !m_ship.contentLock;
    if (Button(MakeRow(left + 156.0f, y + kRowStride * 2.0f, 170.0f, 30.0f), m_ship.allowMissingAssets ? "Missing Assets OK" : "Missing Assets BLOCK")) m_ship.allowMissingAssets = !m_ship.allowMissingAssets;
    if (Button(MakeRow(right, y + kRowStride * 2.0f, 170.0f, 30.0f), m_ship.allowBrokenLinks ? "Broken Links OK" : "Broken Links BLOCK")) m_ship.allowBrokenLinks = !m_ship.allowBrokenLinks;
...
void GameBuilderPanel::HandleCompleteTab(ContentRegistry& registry) {
    ...
    const std::array<std::pair<Rectangle, std::pair<int, std::string*>>, 6> fields {{
        ...
        {MakeRow(left, y + kRowStride), {152, &m_complete.milestoneLabel}},
        {MakeRow(left, y + kRowStride * 2.0f), {153, &m_complete.defaultTemplate}},
        {MakeRow(right, y), {154, &m_complete.defaultProfile}},
        {MakeRow(right, y + kRowStride), {155, &m_complete.defaultBuild}},
        {MakeRow(right, y + kRowStride * 2.0f), {156, &m_complete.releaseProfileId}}
    }};
    ...
    const Rectangle creatorRect = MakeRow(left, y + kRowStride * 3.0f, 510.0f, 30.0f);
    ...
    if (Button(MakeRow(left, y + kRowStride * 4.0f, 220.0f, 34.0f), "Write Completion Bundle")) {
...
void GameBuilderPanel::HandleRegistryTab(ContentRegistry& registry) {
    ...
    if (Button(MakeRow(left + kWideActionColumnOffset, y, 150.0f, 30.0f), "Rebuild Registry")) RefreshRegistry(registry);
    if (Button(MakeRow(left + kWideActionColumnOffset, y + 40.0f, 150.0f, 30.0f), "Write Metadata")) {
        WriteVersionMetadata();
...
void GameBuilderPanel::Draw(Application& app, const ContentRegistry& registry) const {
    ...
    const Rectangle sidebarRect {m_bounds.x + 10.0f, m_bounds.y + 44.0f, kSidebarWidth - 12.0f, m_bounds.height - 96.0f};
    ...
    DrawRectangleRec(sidebarRect, Color{14, 18, 28, 232});
    DrawRectangleLinesEx(sidebarRect, 1.0f, Fade(SKYBLUE, 0.45f));
    DrawRectangleRec(contentRect, Color{10, 14, 22, 228});
    DrawRectangleLinesEx(contentRect, 1.0f, Fade(SKYBLUE, 0.45f));

    DrawBuilderVisibilityText(m_visible);
    DrawText("V128 Builder Label Spacing Rebuild", static_cast<int>(m_bounds.x + 16.0f), static_cast<int>(m_bounds.y + 5.0f), 24, RAYWHITE);
    ...
    DrawText("Tabs stay on the left. Edit fields in the center and click the blue action buttons on the right to create/save content.", static_cast<int>(contentRect.x + 12.0f), static_cast<int>(contentRect.y + 14.0f), 18, LIGHTGRAY);

    const Rectangle actionsHintRect{contentRect.x + contentRect.width - 260.0f, contentRect.y + 12.0f, 248.0f, 60.0f};
    DrawRectangleRec(actionsHintRect, Color{18, 30, 46, 220});
    DrawRectangleLinesEx(actionsHintRect, 1.0f, Fade(SKYBLUE, 0.75f));
    DrawText("Actions", static_cast<int>(actionsHintRect.x + 12.0f), static_cast<int>(actionsHintRect.y + 8.0f), 22, RAYWHITE);
    DrawText("Blue buttons on the right", static_cast<int>(actionsHintRect.x + 12.0f), static_cast<int>(actionsHintRect.y + 32.0f), 18, LIGHTGRAY);

    if (m_activeTab == Tab::Quest) {
        DrawBuilderVisibilityText(m_visible);
        DrawText(TextFormat("Reward Gold: %d", m_quest.rewardGold), static_cast<int>(contentRect.x + 470.0f), static_cast<int>(contentRect.y + 192.0f), 20, GOLD);
    }
    if (m_activeTab == Tab::Fight) {
        DrawBuilderVisibilityText(m_visible);
        DrawText(TextFormat("Count: %d | Radius: %.1f", m_fight.count, m_fight.radius), static_cast<int>(contentRect.x + 12.0f), static_cast<int>(contentRect.y + 260.0f), 20, GOLD);
    }
    if (m_activeTab == Tab::Trade) {
        DrawBuilderVisibilityText(m_visible);
        DrawText(TextFormat("Value: %d | Heal: %d | Stock: %d", m_trade.value, m_trade.healAmount, m_trade.stockCount), static_cast<int>(contentRect.x + 12.0f), static_cast<int>(contentRect.y + 260.0f), 20, GOLD);
    }
    if (m_activeTab == Tab::Persist) {
        DrawBuilderVisibilityText(m_visible);
        DrawText(TextFormat("Starting Gold: %d", m_persist.startingGold), static_cast<int>(contentRect.x + 12.0f), static_cast<int>(contentRect.y + 312.0f), 20, GOLD);
    }
    if (m_activeTab == Tab::Audio) {
        DrawBuilderVisibilityText(m_visible);
        DrawText(TextFormat("Wind %d | Wildlife %d | Fire %d", m_audio.windLevel, m_audio.wildlifeLevel, m_audio.fireLevel), static_cast<int>(contentRect.x + 12.0f), static_cast<int>(contentRect.y + 312.0f), 20, GOLD);
    }
    if (m_activeTab == Tab::Registry) {
        DrawBuilderVisibilityText(m_visible);
        DrawText(TextFormat("Registry Entries: %i", static_cast<int>(registry.Count())), static_cast<int>(contentRect.x + 12.0f), static_cast<int>(contentRect.y + 82.0f), 20, GOLD);
        const auto results = registry.Search(m_registrySearch);
        const Rectangle listRect {contentRect.x + 12.0f, contentRect.y + 118.0f, contentRect.width - 24.0f, std::min(500.0f, contentRect.height - 130.0f)};
        DrawRectangleRec(listRect, Color{12, 16, 24, 220});
        DrawRectangleLinesEx(listRect, 1.0f, Fade(SKYBLUE, 0.7f));
        int drawY = static_cast<int>(listRect.y + 10.0f);
        for (std::size_t i = 0; i < results.size() && i < 18; ++i) {
            ...
            DrawBuilderVisibilityText(m_visible);
            DrawText(line.c_str(), static_cast<int>(listRect.x + 10.0f), drawY, 18, RAYWHITE);
            drawY += 24;
        }
    }
