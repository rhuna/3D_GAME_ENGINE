...
void EditorSceneAuthoring::Update(Application& app, World& world, EditorSelection& selection, PrefabLibrary& prefabs, SceneLibrary& scenes) {
    ...
    if (!mouseLookActive && !mouseOverInspector) {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            selection.BeginBoxSelect(GetMousePosition());
        }

        if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && selection.IsBoxSelecting()) {
            selection.UpdateBoxSelect(GetMousePosition());
        }

        if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT) && selection.IsBoxSelecting()) {
            selection.UpdateBoxSelect(GetMousePosition());
            const Rectangle rect = selection.BoxSelectRect();

            if (rect.width < 6.0f && rect.height < 6.0f) {
                // This was effectively just a click, not a drag selection.
                selection.EndBoxSelect(world, app.GetCamera(), false);

                const Entity picked = selection.Hovered();
                if (picked != 0) {
                    if (ctrlDown) selection.ToggleSelection(picked);
                    else selection.Select(picked);
                } else if (!ctrlDown) {
                    selection.Clear();
                }
            } else {
                selection.EndBoxSelect(world, app.GetCamera(), ctrlDown);
            }
        }
