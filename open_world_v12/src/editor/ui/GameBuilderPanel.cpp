...
void GameBuilderPanel::Update(Application& app, float deltaTime) {
    ...
    if (mouseLeftPressed) {
        const Rectangle createTabButton {kPanelBounds.x + 12.0f, kPanelBounds.y + 74.0f, 138.0f, 32.0f};
        const Rectangle assetsTabButton {kPanelBounds.x + 158.0f, kPanelBounds.y + 74.0f, 138.0f, 32.0f};
        const Rectangle guideTabButton {kPanelBounds.x + 304.0f, kPanelBounds.y + 74.0f, 138.0f, 32.0f};
...
void GameBuilderPanel::Draw(const Application&) const {
    if (!m_visible) {
        return;
    }

    DrawPanelBox(kPanelBounds, "V62 Game Builder", "Create content from forms instead of hand-editing files.");
    DrawTabs();
...
void GameBuilderPanel::DrawGuideTab() const {
    ...
    line("V62 is the first non-coder layer for the engine.");
    ...
    line("F8  Toggle Game Builder panel");
    line("F5  Reload start scene");
    line("F4  Run validation");
    line("TAB Toggle old inspector panel");
    y += 6;
    line("Near-term V63 goal", SuccessColor());
    line("Asset thumbnails + click-to-place scene authoring.");
}
